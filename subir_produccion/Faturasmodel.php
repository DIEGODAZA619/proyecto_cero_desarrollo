...
class Faturasmodel extends CI_Model{
    ...
    public function TodasFaturas($status = false){
        ...
        $this->db->select('f.comprovante, f.id, f.id_usuario, p.nome, p.valor, f.address, f.data_pagamento, f.description'); //DIEGO
        $this->db->from('faturas AS f');
        $this->db->join('planos AS p', 'p.id = f.id_plano', 'inner');
                        ...
                        if(!empty($this->rede_total)){
                            ...
                            foreach($this->rede_total as $nivel=>$patrocinador){
                                ...
                                if(isset($this->todos_niveis[$nivel+1])){
                                    ...
                                    if($ganancias > 0)
                                    {
									   GravaExtrato($patrocinador, $ganancias, 'User Referral Bonus  '.InformacoesUsuario('login',$row->id_usuario), 1);
                                    }
                                }
                            }
                        }
                ...
                if(!empty($this->rede_total)){
                    ...
                    foreach($this->rede_total as $patrocinadores){
                        ...
                        $rede = $this->db->get('rede');
						 	...
						 	//echo "<br>ID: ".$patrocinadores ." | puntos: ".$row->plano_carreira." | lado ".$LadoChaveBinaria;
							/*edward nueva tabla control puntos eliminar*/
                        ...
                        /* edward nueva tabla control puntos eliminar */
                        $qx = $this->db->get_where('rede', ['id_usuario' => $row->id_usuario]);
                        $us = $qx->row();
                        $pagar = true;
                        if ($patrocinadores == $us->id_patrocinador_direto) {
                            $redes = $this->getPatrocinadorDir($patrocinadores, $us->chave_binaria, $row->id_usuario);
                            if (count($redes) > 0) {
                                //pagar
                                $pagar = true;
                            } else {
...
redirect('admin/faturas/liberar');
    ...
    function getPatrocinadorDir($id_patrocinador, $llave, $id_user) {
        $query = $this->db->query('select * from rede 
            left join faturas f on f.id_usuario = rede.id_usuario
            left join planos p on p.id = f.id_plano
            where p.valor > 0 and id_patrocinador_direto = ' . $id_patrocinador . ' and f.status = 1 and chave_binaria = ' . $llave . '  and rede.id_usuario != '.$id_user);
        return $query->result();
    }
